...
Marka OZMO = **spokojne, niezawodne narzędzie operacyjne**. Panel korzysta z grafitowej szyny,
jasnego płótna i terakoty dla głównych działań. Pastelowe żółty, różowy, zielony i błękitny są
zarezerwowane dla podsumowań i statusów — nigdy jako dekoracyjny gradient. Strategia produktu
pozostaje restrained; dane, nie ozdobniki, budują charakter.

### 12.2 Powierzchnie — hierarchia, która naprawia „zlewanie się"
Trzy celowe tony (fundament redesignu):
- `--background` — **canvas** aplikacji, ciepła biel łamana `oklch(0.975 0.004 75)`.
- `--card` / `--popover` — **powierzchnie** nad canvasem, niemal białe `oklch(0.995 0.002 75)`.
...
polskie znaki.
- **Nagłówki — Bricolage Grotesque Variable** (`--font-heading`): zwarty, rozpoznawalny głos bez
  ozdobności; `letter-spacing: -0.02em`, `text-wrap: balance`.
- **UI/body — Inter Variable** (`--font-sans`): neutralny i czytelny przy dużej gęstości danych.
...
- Zakazane (nie przywracać): fioletowo-niebieskie gradienty AI, gradient-text, glassmorphism
  ...
  sekcją, karta-w-karcie, kolorowe paski-boczne, fonty z CDN.

### 12.8 Eksport tokenów

Przenośne tokeny są w `tokens.css`. Tailwind mapuje je w
`app/assets/css/tailwind.css` przez `@theme inline`, a zmienne shadcn wskazują te same role.
Nie utrzymujemy osobnego `tokens.json`, bo bez pipeline DTCG byłby martwą kopią źródła prawdy.
